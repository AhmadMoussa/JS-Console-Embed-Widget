(function() {
    const Widget = Object.create({
        create(id) {
            ...
            wdg.innerHTML = `<header><style>.logFrame-${ id },.runButton-${ id },.textArea-${ id }{border:1px solid #999;margin:2px;padding:3px}.widgetContainer-${ id }{display:flex;flex-direction:row;justify-content:space-between}.buttonAndInput-${ id }{display:flex;flex-direction:column}.logFrame-${ id }{flex:1 0}.textArea-${ id }{flex:1 0;resize:horizontal}</style></header><div class=\"widgetContainer-${ id }\"><div class=\'buttonAndInput-${ id }\'><textarea id=\'code-${ id }\' class=\'textArea-${ id }\'></textarea><button id=\'safe-${ id }\' class=\'runButton-${ id }\'>Run</button></div><iframe sandbox=\'allow-scripts\' id=\'sandboxed-${ id }\' class=\'logFrame-${ id }\'></iframe></div>`;
